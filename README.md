...
- [资源替换指南](#资源替换指南)
  ...
  - [各类型替换细节说明](#各类型替换细节说明)
- [工具与扩展功能](#工具与扩展功能)
  - [剧情文本打包工具](#剧情文本打包工具-fgsb-tool)
  - [自定义加载图像与动画](#自定义加载图像与动画)
  - [剧情文本语音接口](#剧情文本语音接口-scriptvoice)
  - [超分图集自动下载](#超分图集自动下载-autodownload)
- [常见问题 (FAQ)](#常见问题-faq)
- [相关链接](#相关链接)
...
国服无需翻译文件，所以无需下载mapping仓库的内容。只需要开启 `TextReplace`、`MasterDataReplace`、`LocalizationReplace` 与 `ScriptReplace` 即可进行简单的文本替换。
...
AutoCommandCardNpCardTextDownload=0 # 自动下载中文宝具卡文本图像 (国服无需开启)
AutoCommandCardSvtImgDownload=0     # 自动下载指令卡从者贴图 (仅调试)
...
>
> 动图首次加载会在后台生成缓存，此时系统会卡顿，属于正常现象，等一段时间生成完成后会就恢复了
...
1. **从者卡面、头像与立绘**
   - **通用命名规范**（适用于卡面/窄卡面/头像/状态立绘）：
     ...
     - **若还是搞不清楚怎么命名的，开启autodownload，自动下载后去autodownload文件夹看看文件是什么命名**
   - **CharaFigure**：
...
5. **指令卡宝具文本与人物贴图替换**
   ...
   - **指令卡宝具文本替换命名规范**：
     - **A · 标准**
       - 运行时贴图名：`card_servant_np`
       - 本地文件：`NpText/<svtId>.png`
     - **B · 灵基覆写**
       - 运行时贴图名：`405910`（纯数字）
       - 本地文件：`NpText/<svtId>@405910.png`
     - **C · 变身**
       - 运行时贴图名：`card_servant_np_304802`
       - 本地文件：`NpText/<svtId>@304802.png`
     - **D · 序号**
       - 运行时贴图名：`card_servant_np4`
       - 本地文件：`NpText/<svtId>@4.png`
   - **示例**：
