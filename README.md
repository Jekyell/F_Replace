...
#### 方法一：自动更新（推荐）
启动 App 进入主界面，模块会自动联网下载并增量更新汉化文件。可在右上角文件列表中勾选需忽略更新的文件或文件夹。
...
国服玩家可开启 `TextReplace`、`MasterDataReplace`、`LocalizationReplace` 与 `ScriptReplace` 进行文本替换。
- **替换词下载**：下载 [Telegram 替换词文件](https://t.me/fgomod/598) 后放入 `Mod/F_Replace` 目录。
...
MasterDataReplace=1       # MasterData 数据替换
...
├── Figure/
│   ├── CharaFigure/        # 从者战斗立绘 (Form/ 特定形态, Lock/ 表情锁定)
│   ├── CharaGraph/         # 从者大卡面
│   ├── Faces/              # 从者头像
...
> [!WARNING]
> 卡面与动态卡面 (MP4/WebP) 会增加显存和内存开销。建议静态图保持原图 2 倍超分，动态卡面控制在 30 帧以内，避免引发内存溢出或闪退。

### 资源格式与优先级速查

| 资源类型 | 对应目录 | 支持格式 | 格式读取优先级 | 备注 |
| :--- | :--- | :--- | :--- | :--- |
| **大卡面** (`CharaGraph`) | `Figure/CharaGraph/` | `.mp4`, `.webp`, `.png` | `mp4` → `webp` → `png` | 支持动态卡面 |
| **半身卡面** (`NarrowFigure`) | `Figure/NarrowFigure/` | `.mp4`, `.webp`, `.png` | `mp4` → `webp` → `png` | 四角切角 |
| **头像** (`Face`) | `Figure/Faces/` | `.mp4`, `.webp`, `.png` | `mp4` → `webp` → `png` | 左上+右上切角 |
| **状态栏立绘** (`Status`) | `Figure/Status/` | `.png`, `.astc`, `.astc.zstd` | `png` → `astc` → `astc.zstd` | 不支持动图 |
| **战斗立绘** (`CharaFigure`) | `Figure/CharaFigure/` | `.png`, `.astc`, `.astc.zstd` | `png` → `astc` → `astc.zstd` | 不支持动图 |

### 各类型替换细节说明

1. **从者卡面与头像**
   - **命名规范**：`[从者 ID]@[突破数]`（突破数范围 `1`~`4`，灵衣从 `11` 开始）。例如 `4000100@4.png`（奥尔加玛丽满破卡面）、`204300@1.png`（芭万希第一阶段头像）。
   - **特定形态**：部分特殊立绘存放在 `Figure/CharaFigure/Form/[FormID]/` 子目录中。
...
2. **御主头像与立绘 (`Master`)**
   ...
   - **文件要求**：头像使用 `face.mp4` 或 `face.png`（`mp4` 优先）；主立绘为 `figure.png`；蒙版遮罩为 `figure_a.png`。

3. **模型与宝具资源**
   - **战斗模型**：放入 `Models/` 目录，加载战斗时自动替换贴图或 Model 资源。
   - **宝具动画**：放入 `Np/` 目录，自动替换对应宝具资源。
...
- **读取逻辑**：打包文件存放在 `Mod/ScriptBundle/`。模块会优先读取 `Mod/Script/` 下的 `.txt` 解压文件。推荐直接使用 App 自动拉取。
...
- [Telegram 动态发布频道](https://t.me/fgomod)
- [Telegram 全从者超清卡面图集](https://t.me/fgomod_1)
- [模型修改教程 (models.md)](https://github.com/Jekyell/F_Replace/blob/main/models.md)
