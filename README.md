...
与原有的单txt剧情文本替换功能兼容，**模块会优先读取 Mod/Script/ 目录下的 txt 文件，因此推荐使用app拉取即可，非必要情况下无需使用fgsb文件**
...
使用说明： 统一命名规范为 [从者id]@突破数，如奥尔加玛丽满破：4000100@4（突破数范围为 1~4，灵衣从11开始）。
...
* **存放路径**：`Mod/Models/`
* **使用说明**：放入修改后的贴图文件或整个模型文件，游戏加载战斗模型时会自动替换。
